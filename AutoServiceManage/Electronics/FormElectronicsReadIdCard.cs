...
{
    public partial class FormElectronicsReadIdCard : Form
    {
        #region 构造函数,load,窗体事件

        public FormElectronicsReadIdCard()
        {
            InitializeComponent();
        }
        ...
        {
            LogService.GlobalInfoMessage("进入readIDCrad_OnReadedInfo方法");
            LogService.GlobalInfoMessage(isShowFrom+"");
            if (isShowFrom == false)
            {
                ...
                LogService.GlobalInfoMessage(e.NewHuman.Name);
